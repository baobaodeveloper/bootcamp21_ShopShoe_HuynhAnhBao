...
const shoeReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case "ADD_SHOPPING_CARD": {
      ...
      const index =
        arrNewShopCart.length > 0 &&
        arrNewShopCart.findIndex((item) => item.id === action.item.id);
      if (index === -1) {
        arrNewShopCart.push(action.item);
      } else if (index) {
        arrNewShopCart[index].price =
          arrNewShopCart[index].price / arrNewShopCart[index].qty +
          arrNewShopCart[index].price;
        arrNewShopCart[index].qty += 1;
      }
      state.shopCard = arrNewShopCart;

      return { ...state };
    }
    case "SHOW_SHOPPING_CARD": {
      return state;
    }
    case "QTY_INCREA": {
